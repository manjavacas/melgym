...
the objetive is to maintain CV002 pressure as close as possible to a given setpoint (by default, 101000 Pa).

CF is the control function to be learned by the agent, while the reward corresponds to the distance to the provided setpoint.
"""
...
class EnvPress(Env):
    """
    Pressure control environment. Outlet air velocity is adjusted according to a CV target pressure.
    """
    ...
    def __init__(self, input_file, n_actions, controlled_cvs, control_horizon=10, check_done_time=500, max_deviation=500, min_velocity=0, max_velocity=20, max_tend=10_000, render_mode=None, ylims=None, time_bt_frames=0.01, env_id=None):
        ...
        if render_mode:
            ...
            if self.render_mode == 'pressures':
                ...
                _, self.ax = plt.subplots(figsize=(14, 8))
                ...
                for record in self.ys:
                    self.graphs.append(
                        self.ax.plot(self.x, record['values'], label=record['cv'])[0])

                self.ax.axhline(y=101000, color='r', alpha=0.5, linestyle='--', label='Target pressure')

                ## ajust legend and ylims
                self.ax.legend(loc='upper center', bbox_to_anchor=(
